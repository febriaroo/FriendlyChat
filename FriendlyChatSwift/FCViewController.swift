...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    func configureAuth() {
        ...
        _authHandle = Auth.auth().addStateDidChangeListener { (auth: Auth, user: User?)in
            ...
            //check if current user same with the one at fb db
            if let activeUser = user {
                //check if the current app user not same with in FIRUser
    ...
    func loginSession() {
//        let authUI = FUIAuth.init(auth: Auth.auth())
...
//        authUI?.delegate = self as! FIRAuthUIDelegate
//        //authUI?.providers = [googleProvider, facebookProvider]
    ...
    func sendPhotoMessage(photoData: Data) {
        ...
        //add node to firebase DB
        storageRef!.child(imagePath).putData(photoData, metadata: metadata){ (metadata, error) in
            if let error = error {
                print ("Error Uploading: \(error)")
                return
            }
            //use sendMessage to add imageURL to DB
